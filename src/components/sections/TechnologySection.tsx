...
export function TechnologySection() {
  ...
  return (
    <section id="technology" className="scroll-mt-20 md:scroll-mt-24">
      ...
      <div className="section-y grid-faint">
            ...
            <div className="relative">
              ...
              <ol className="space-y-5">
                {t.technology.layers.map((l, i) => (
                  <Reveal as="li" key={l.title} delay={i * 0.08} className="relative pl-12">
                    <span
                      aria-hidden="true"
                      className="glow-text absolute top-6 left-0 flex h-7 w-7 items-center justify-center rounded-full border border-cyan bg-abyss font-mono text-[10px] text-cyan"
                    >
                      {count - i}
                    </span>
                    <div className="surface-card p-6">
                      <h4 className="text-lg font-semibold">{l.title}</h4>
                      <p className="mt-3 text-sm leading-relaxed text-steel">{l.text}</p>
                    </div>
                  </Reveal>
                ))}
              </ol>
      ...
      <div className="section-y border-y border-ocean/50 bg-navy/20">
        <div className="mx-auto max-w-[1400px] px-5 md:px-10">
          <Reveal>
