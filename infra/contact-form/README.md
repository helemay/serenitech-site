...
AWS Lambda Function URL; the function e-mails the request through Amazon SES and returns
`{"ok": true}`.
...
| IAM role | `serenitech-contact-form-role` — inline policy `ses-send-and-logs` (ses:SendEmail on the `serenitech.global` identity + CloudWatch Logs) |
| Function URL | `https://xblwde2qf4gihsfbmgbuaceyfm0kvfio.lambda-url.eu-west-3.on.aws/` — auth NONE, CORS: POST from https://serenitech.global, https://www.serenitech.global, https://serenitech.lovable.app |
...
are silently accepted and dropped; field length limits; Origin allow-list (also enforced by
the Function URL CORS config). Reply-To is set to the visitor's address, so replying from
...
```bash
RECIPIENTS="a@x,b@y" ./deploy.sh      # creates or updates role, function, URL; prints the URL
```
